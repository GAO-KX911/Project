...
TriggerMode = 0  # 记录触发模式状态的占位符，避免未定义报错 

def check_ret(ret, msg):
    if ret != MV_OK:
        raise RuntimeError(f"{msg} failed: 0x{ret:x}")
...
class HikCameraController:
    ...
    def initialize_camera(self):
        ...
        try:
            ...
            # 设置相机参数
            # 保持连续取流，软触发用于 IO 时临时切换
            self.cam.MV_CC_SetEnumValue("TriggerMode", 0)  # 连续采集
            self.cam.MV_CC_SetEnumValue("ExposureAuto", 0)  # 手动曝光
            self.cam.MV_CC_SetFloatValue("ExposureTime", 1850.0)  # 曝光时间

            # 配置 IO：Line1 软触发频闪输出（默认脉宽 300ms，可按需调整）
            # try:
            #     self.init_io_strobe(line="Line1", pulse_ms=300)
            # except Exception as io_err:
            #     print(f"IO 配置失败（忽略继续取流）: {io_err}")
    ...
    def init_io_strobe(self, line="Line1", pulse_ms=200):
        """将指定线路配置为软触发频闪输出"""
        cam = self.cam
        if cam is None:
            raise RuntimeError("camera not initialized")

        duration_us = int(pulse_ms * 1000)

        # 选择线路
        ret = cam.MV_CC_SetEnumValueByString("LineSelector", line)
        if ret != MV_OK:
            raise RuntimeError(f"LineSelector {line} 设置失败: 0x{ret:x}")

        # 线路模式：频闪输出（部分机型无此节点，失败忽略）
        LineMode = "Strobe"
        if self.cam.MV_CC_SetEnumValueByString("LineMode", LineMode) == MV_OK:
            print(f"[INFO] LineMode = {LineMode}")
        else:
            print("LineMode is unknown")
        # 选择输出源：ExposureStartActive/ExposureActive
        ok = False
        for src in ["ExposureStartActive", "ExposureActive"]:
            if self.cam.MV_CC_SetEnumValueByString("LineSource", src) == MV_OK:
                ok = True
                print(f"[INFO] LineSource = {src}")
                break
        if not ok:
            raise RuntimeError("LineSource cannot be set to ExposureStartActive/ExposureActive on this camera")

        # 使能 strobe + 设置脉宽（us）
        check_ret(self.cam.MV_CC_SetBoolValue("StrobeEnable", True), "StrobeEnable=True")
        check_ret(self.cam.MV_CC_SetIntValue("StrobeLineDuration", duration_us), f"StrobeLineDuration={duration_us}us")
        self.io_configured = True
        print(f"[INFO] Line1 strobe pulse width ≈ {pulse_ms} ms")

    def fire_pulse(self, repeat=1, interval_s=0.6):
        """
        触发一次（或多次）软触发事件，让 Line1 输出脉冲。
        在连续取流基础上临时切换到软触发，再切回。
        """
        if not self.cam:
            print("[IO] fire_pulse: camera not initialized")
            return
        if not self.io_configured:
            try:
                self.init_io_strobe()
            except Exception as e:
                print(f"[IO] fire_pulse: IO 配置失败 {e}")
                return

        try:
            self.cam.MV_CC_StopGrabbing()
            check_ret(self.cam.MV_CC_SetEnumValue("TriggerMode", 1), "TriggerMode=On")
            check_ret(self.cam.MV_CC_SetEnumValueByString("TriggerSource", "Software"), "TriggerSource=Software")
            check_ret(self.cam.MV_CC_StartGrabbing(), "StartGrabbing")

            for i in range(repeat):
                print(f"[IO] SoftTrigger {i+1}/{repeat}")
                check_ret(self.cam.MV_CC_SetCommandValue("TriggerSoftware"), "TriggerSoftware")
                time.sleep(interval_s)
        except Exception as e:
            print(f"[IO] fire_pulse failed: {e}")
        finally:
            try:
                self.cam.MV_CC_StopGrabbing()
            except:
                pass
            check_ret(self.cam.MV_CC_SetEnumValue("TriggerMode", 0), "TriggerMode=Off")
            print(f"TriggerMode = {TriggerMode}")
            
            # 恢复 LineMode 为 Strobe（保持脉冲模式）
            check_ret(self.cam.MV_CC_SetEnumValueByString("LineMode", "Strobe"), "LineMode=Strobe")
            
            # 设置 LineSource 为 ExposureStartActive 或 ExposureActive
            ok = False
            for src in ["ExposureStartActive", "ExposureActive"]:
                if self.cam.MV_CC_SetEnumValueByString("LineSource", src) == MV_OK:
                    ok = True
            ...
            self.cam.MV_CC_StartGrabbing()  # 恢复抓取
            ...
            function updateStatus() {
                fetch('/camera/status')
                    ...
                    .then(data => {
                        ...
                        const uptime = data.system_stats ? Math.round(data.system_stats.uptime) : 0;
                        const captureCount = data.system_stats ? data.system_stats.capture_count : 0;
                        
                        statusDiv.innerHTML = `
                            <div class="${statusClass}">
                                <strong>相机状态:</strong> ${data.camera_connected ? '✅ 已连接' : '❌ 未连接'} | 
                                <strong>运行时间:</strong> ${uptime}秒 | 
                                <strong>捕获数量:</strong> ${captureCount}
                            </div>
                        `;
                    })
                    .catch(error => {
                        document.getElementById('status').innerHTML = 
                            '<div class="status offline">状态查询失败</div>';
                    });
            }
...
@app.route('/camera/fire_pulse', methods=['POST'])
def fire_pulse_api():
    """触发相机 IO 脉冲"""
    if not camera_controller.is_connected:
        if not camera_controller.initialize_camera():
    ...
    try:
        camera_controller.fire_pulse(repeat=repeat, interval_s=interval)
        return jsonify({
            "success": True,
            "message": f"已触发 {repeat} 次软触发",
            "repeat": repeat,
            "interval": interval
        })
    except Exception as e:
