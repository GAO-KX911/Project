...
class DFFTNet_new(nn.Module):
    def __init__(self,
        ...
        # self.temp_dot = torch.nn.Conv2d(a, b, ks, stride, pad, dilation, groups, bias=False)
    ...
    def forward(self, x, filename_list):
        ...
        # template操作
        temp = template_construct(filename_list, th, tw)
        # print(temp.shape)
        #
        # print(temp.size(0), temp.size(1), temp.size(2), temp.size(3))
        # print("***********************************")
        ...
        temp_feature = []
        # temp_dot_stage1 = self.layers[0]
        # print(isinstance(temp_dot_stage1, DOTBlock))
        # print("****************************************")
        # print("333temp", temp.shape, temp_Wh, temp_Ww)
        # temp, temp_H, temp_W = self.temp_dot_stage1(temp, temp_Wh, temp_Ww)
        # print(temp.shape)
        # print("1****************************************")

        # B, _, C = temp.shape
        # cross_temp = temp.view(B, temp_H, temp_W, C).permute(0, 3, 1, 2).contiguous()
        # conv_layer = getattr(self, f'saaconv{len(temp_feature)}')
        # if len(temp_feature) < 2:
        #     cross_temp = conv_layer(cross_temp)
        # temp_feature.append(cross_temp.contiguous().view(B, channel[len(temp_feature)], -1).transpose(-2, -1))
        # print(temp.shape)
        # print("2****************************************")
    ...
    def train(self, mode=True):
        ...
        super(DFFTNet_new, self).train(mode)
        self._freeze_stages()
# 
